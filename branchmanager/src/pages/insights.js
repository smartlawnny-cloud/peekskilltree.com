...
var InsightsPage = {
  render: function() {
    ...
    var funnelData = [
      { label: 'Clients', count: totalLeads, color: '#2196f3' },
      { label: 'Requests', count: totalRequests, color: '#42a5f5' },
      { label: 'Quotes', count: totalQuotes, color: '#66bb6a' },
      { label: 'Converted', count: convertedQuotes, color: '#4caf50' },
      { label: 'Jobs', count: totalJobs, color: '#2e7d32' },
      { label: 'Completed', count: completedJobs, color: '#1b5e20' }
    ];
    ...
    funnelData.forEach(function(f) {
      ...
      html += '<div style="margin-bottom:8px;">'
        + '<div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:2px;"><span>' + f.label + '</span><strong>' + f.count + '</strong></div>'
